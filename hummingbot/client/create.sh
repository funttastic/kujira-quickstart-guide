...
then
  ...
  if [ "$RESPONSE" == "" ]
  then
    read -p "   Enter the image you want to use (default = \"hb-client\") >>> " RESPONSE
  fi
  if [ "$RESPONSE" == "" ]
  then
  ...
  if [ "$RESPONSE" == "" ]
  then
    read -p "   Enter a name for your new instance (default = \"hb-client\") >>> " RESPONSE
  fi
  if [ "$RESPONSE" == "" ]
  then
...
CERTS_FOLDER="$COMMON_FOLDER/certs"
